...
def version():
    ...
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_VERSION]
...
def bugreport(dest_file="default.log"):
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_BUGREPORT]
    try:
...
def push(src, dest):
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_PUSH, src, dest]
    return _exec_command(adb_full_cmd)
...
def pull(src, dest):
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_PULL, src, dest]
    return _exec_command(adb_full_cmd)
...
def devices(opts=[]):
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_DEVICES, _convert_opts(opts)]
    return _exec_command(adb_full_cmd)


def shell(cmd, udid=None):
    """
    Execute shell command on target
    :param cmd: string shell command to execute
    :param udid: device serial number
    :return: result of _exec_command() execution
    """
    if udid is None:
        adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_SHELL, cmd]
    else:
        adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_ARG_ID, udid, Cmd.ADB_COMMAND_SHELL,
                        cmd]
    return _exec_command(adb_full_cmd)
...
def connect(ip):
    """
    Connect adb to specified ip and port 
    :param ip: ip address of the device
    :return: the device udid
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_TCPIP, '5555']
    _exec_command(adb_full_cmd)
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_CONNECT, '%s:5555' % ip]
    _exec_command(adb_full_cmd)
    return '%s:5555' % ip
...
def install(apk, opts=[]):
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_INSTALL, _convert_opts(opts), apk]
    return _exec_command(adb_full_cmd)
...
def uninstall(app, opts=[]):
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_UNINSTALL, _convert_opts(opts), app]
    return _exec_command(adb_full_cmd)
...
def getserialno():
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_GETSERIALNO]
    return _exec_command(adb_full_cmd)
...
def wait_for_device():
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_WAITFORDEVICE]
    return _exec_command(adb_full_cmd)
...
def sync():
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_SHELL, Cmd.ADB_COMMAND_SYNC]
    return _exec_command(adb_full_cmd)
...
def start_server():
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_START_SERVER]
    return _exec_command(adb_full_cmd)
...
def kill_server():
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_KILL_SERVER]
    return _exec_command(adb_full_cmd)
...
def get_state():
    ...
    :return: result of _exec_command() execution
    """
    adb_full_cmd = [Cmd.ADB_COMMAND_PREFIX, Cmd.ADB_COMMAND_GET_STATE]
    return _exec_command(adb_full_cmd)
